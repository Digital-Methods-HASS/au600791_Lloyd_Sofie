...
#It can be read when running head that we are working with a projected CRS, thus we are working with 2D maps.
...
**Answer**  
The measurement units of the `cities_sf3035` object is metre and can be seen when typing st_crs(cities_sf3035).
...
current.mode <- tmap_mode(mode = c("view"))
tm_shape(ten_cities) +
  tm_text("Ancient Toponym", size = 0.7, auto.placement = 5) +
  tm_dots(col = "orange", 
             size = 0.1) +
...
**Answer**  
The insceptions are from Italy.
...
**Answer**  
Because the EPSG number is a single numeric reference to both the datum and
projection. And because we want to have the same CRS in all our objects.
...
# Inspect results
head(insc_urban) # The insc_urban is polygons
head(insc_rural) # The insc_rural is geometry

# Calculate ratio - this should work, but it don't....
...
**Answer**  
No idea... I can't get my code to work, it just says 'NaN' or maybe I'm doing it wrong... 

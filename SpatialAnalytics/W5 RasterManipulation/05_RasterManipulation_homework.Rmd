...
inMemory(elevation) #False - don't know why
...
# Check again if CRS matches
st_crs(mounds_32635) == st_crs(elevation)

# Create a bounding box around the mounds with `st_make_grid()`
...
res(east) # Resolution is 10/10

# Determine the number of cells
ncell(east) # There is 2374596 cells
...
res(east_small) # Resolution is 100/100

# Determine the number of cells in the new raster
ncell(east_small) # There is 23940 cells
```
...
* Find or recreate the `mounds` sf object, `prominence` and `elevation_prom_overlay` rasters.
* Plot the `elevation_prom_overlay` and the `mounds` on top of each other to check visual overlap. Do the same with `prominence` raster and `mounds`.
...
# Plot the ´elevation_prom_overlay´ and the `mounds` on top of each other to check visual overlap
plot(elevation_prom_overlay)
plot(st_geometry(mounds),add=TRUE)

# Plot the `prominence` and the `mounds` on top of each other to check visual overlap
plot(prominence)
plot(st_geometry(mounds),add=TRUE)
...
# Using ncell to find out how many there are 
ncell(mounds_high_vis) # There is 369930 cells
...
unique(mounds_25$TRAP_Code) # Run code to see the different trap code IDs
